...
export default function MultiSelect({ filterValues, tableColumn, children }) {
    ...
    return (
        <div className='inline-block relative'>
            ...
            <div className={`absolute bg-white p-2 shadow-[0px_5px_15px_rgba(0,0,0,0.35)_;] rounded top-[43px] w-[170px] transition-all duration-200 ${isOpen ? 'visible opacity-100' : 'opacity-0 invisible'} `}>{renderedCheckBoxes}</div>
        </div>
    )
}
